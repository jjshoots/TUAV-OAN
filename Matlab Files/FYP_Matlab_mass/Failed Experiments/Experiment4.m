...
while focus(1) ~= targ(1) && focus(2) ~= targ(2)
    for i = 1:4
        ...
        OPEN_LIST(i, :) = [focus+steps(i, :) 99999 99999 99999]; % x y G H F, note F = G+H
    end
    
    for i = 1:4
        %if adjacent tiles exist within nodes (not an observed obstacle OR
        %inaccessible path)
        if sift([OPEN_LIST(i, 1:2)], NODES) && ~sift([OPEN_LIST(i, 1:2)], no_pass) && i ~= wrong_direction
            % compute cost to get there based on previous cost
            G = current_G + 1;
            % compute heuristic
            H = norm(OPEN_LIST(i, 1:2) - targ);
            % sum cost and heuristic
            F = G + H;
            % add adjacent tile and costs to open list
            OPEN_LIST(i, 3:5) = [G H F];
        end
    end
    ...
    if throw == 99999 || sift([OPEN_LIST(idx, 1) OPEN_LIST(idx, 2)], CLOSED_LIST(:, 1:2))
        % if open list is pointless, ie: no available paths
        % move back one point and solve from there
        no_pass = [no_pass; focus];
        ...
        CLOSED_LIST = [CLOSED_LIST; OPEN_LIST(idx, 1:5)];
        %current cost
        current_G = current_G + 1;
    end
end
